$(document).bind("mobileinit", function(){
    $.mobile.notesdb = openDatabase("Attractions", "1.0", "Osaks Attractions Map", 2*1024*1024);
    $.mobile.notesdb.transaction(function (t) {
	    t.executeSql("CREATE TABLE IF NOT EXISTS Attractions (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL,details TEXT NOT NULL, created TEXT NOT NULL, updated TEXT, latitude REAL, longitude REAL, pic TEXT);");
		//
	  ...
	  //t.executeSql('DROP TABLE Food');
		...
		if (isLoadData==null){  // 第一次載入預設的資料
			...
			$.each(AttractionsData, function(InfoIndex, Info) {
				var title = Info["title"];
				var details = Info["details"];
				var lat = Info["latitude"];
				var lng = Info["longitude"];
				var pic = Info["pic"];
				t.executeSql('INSERT into Attractions(title, details, created, latitude, longitude, pic) VALUES (?,?,date("now"),?,?,?);',[title, details, lat, lng, pic]);
		    })
		}
    });
});
...
if (FoodNote.lat==null || FoodNote.lng==null ){
	FoodNote.lat=34.675099;  // 開始時，以文章牛肉湯為中心點
	FoodNote.lng=135.500509;
...
function getMap() {  // 顯示地圖和地標
	...
	//在地標上加入事件
	$.mobile.notesdb.transaction(function(t) {
	    t.executeSql("SELECT id, title,details, pic, latitude,longitude FROM Attractions ORDER BY id DESC LIMIT ?", [FoodNote.limit], function(t, result) {
		...
		if (len > 0 ) {
			for (i = 0; i < len; i += 1) {
				...
				google.maps.event.addListener(marker[i], "click", function(event) {
					...
					// 取得按下的地標
					$.mobile.notesdb.transaction(function(t) {
						t.executeSql("SELECT id, title,details,latitude,longitude FROM Attractions ORDER BY id DESC LIMIT ?",
...
function getTitles() {
	...
	var items = [];
	$.mobile.notesdb.transaction(function(t) {
		t.executeSql("SELECT id, title,details,latitude,longitude,pic FROM Attractions ORDER BY id DESC LIMIT ?", [FoodNote.limit], function(t, result) {
...
function getItem(id) {
	$.mobile.notesdb.transaction(function(t) {
		t.executeSql("SELECT * FROM Attractions WHERE id = ?", [id], function(t, result) {
...
function insertItem(e) { // 新增一個景點資料
	...
	if (title==""){
		...
		$.mobile.notesdb.transaction(function(t) {
			t.executeSql('INSERT into Attractions(title, details, created, latitude, longitude) VALUES (?,?,date("now"),?,?);',
...
function updateItem(e) {  // 修改指定 id 的資料
	...
	$.mobile.notesdb.transaction(function(t) {
		t.executeSql('UPDATE Attractions SET title = ?, details = ?, updated = date("now"), latitude=?, longitude=? WHERE id = ?',
...
function deleteItem(e) {  // 刪除指定 id 的資料
	...
	if(flagConfirm) {
		var id = $(this).attr("data-trnote");
		$.mobile.notesdb.transaction(function(t) {
			t.executeSql("DELETE FROM Attractions WHERE id = ?", [id],
